...
The lower-level `sizing/engine_sizing.py` module is retained as a reusable
constraint-equation helper. It provides the shared thrust-to-weight equation
used by both `sizing/constraint_diagram.py` and `sizing/mission_closure.py`.
The old standalone `_engine_sizing_constraints.png` output has been removed in
favor of the integrated `_constraint_diagram.png` chart.
