...
## Duality Constraint Sizing

This branch includes a Duality aircraft sizing workflow under `sizing/` that
couples the aircraft weight and volume model to engine-deck-based constraint
analysis.

![Duality engine cycle station map](duality_cycle_map.png)

The Duality cycle station map is generated by:

    python example_cycles/duality.py

The current constraint diagram is generated by:

    python -m sizing.constraint_diagram

That script writes `_constraint_diagram.png` and evaluates the nine literature
constraint cases used for thrust-loading versus wing-loading sizing:
...
The lower-level `sizing/constraint_equations.py` module provides the shared
thrust-to-weight equation used by both `sizing/constraint_diagram.py` and
`sizing/mission_closure.py`. The old standalone engine-sizing script and its
`_engine_sizing_constraints.png` output have been removed in favor of the
integrated `_constraint_diagram.png` chart.
