...
def plot_cycle_map(prob):
    ...
    for pt, cfg in MODES.items():
        ...
        all_data[pt] = dict(Tt=np.array(Tt), Ts=np.array(Ts),
                            Pt=np.array(Pt), Ps=np.array(Ps),
                            MN=np.array(MN), A=np.array(A))
    ...
    for col, (pt, cfg) in enumerate(MODES.items()):
        ...
        for row, (k_tot, k_stat, lbl_tot, lbl_stat, c_tot, c_stat) in enumerate(ROW_KEYS):
            ...
            ax.plot(xs, _mask(d[k_tot]), color=c_tot, marker='o', linewidth=2,
            ...
            if k_stat is not None:
                ax.plot(xs, _mask(d[k_stat]), color=c_stat, marker='s', linewidth=2,
